...
After running `.generate_world()`, we can run Karel's actions and
finally visualize it all with the function
[`.run_actions()`](https://docs.ropensci.org/karel/reference/dot-run_actions.md).
Thiese are all internal functions, users just call the corresponding
external wrapper according to their language.
